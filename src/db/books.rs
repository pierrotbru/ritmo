...
impl Book {
    pub async fn add_book(&mut self, pool: SqlitePool) -> Result<i32, RitmoErr> {
        let mut tx = pool
            .begin()
            .await
            .map_err(|e| RitmoErr::DatabaseConnectionFailed(e.to_string()))?;

        let mut new_book = BookDbData {
            name: self.data.name.clone(),
            publication_date: self.data.publication_date.clone(),
            acquisition_date: self.data.acquisition_date.clone(),
            last_modified_date: self.data.last_modified_date.clone(),
            series_index: self.data.series_index.clone(),
            original_title: self.data.original_title.clone(),
            notes: self.data.notes.clone(),
            has_cover: self.data.has_cover.clone(),
            has_paper: self.data.has_paper.clone(),
            file_link: self.data.file_link.clone(),
            ..Default::default()
        };

        if let Some(publisher) = &self.data.publisher {
            ...
            .await
            .map_err(|e| RitmoErr::SearchAndAddFailed(format!("Failed to search and add publisher {}: {}", publisher, e)))?;
        ...
        if let Some(format) = &self.data.format {
            ...
            .await
            .map_err(|e| RitmoErr::SearchAndAddFailed(format!("Failed to search and add format {}: {}", format, e)))?;
            new_book.format_id = result.id;
        }

        if let Some(series) = &self.data.series {
            let result = search_and_add(
                &mut tx,
                "series",
                "id",
            ...
            .await
            .map_err(|e| RitmoErr::SearchAndAddFailed(format!("Failed to search and add series {}: {}", series, e)))?;
        ...
        let result = query!(
            ...
            new_book.acquisition_date,
            new_book.last_modified_date,
            new_book.series_id,
...
//        dbg!(&result);
        ...
        for tag in &self.data.tags.clone() {
